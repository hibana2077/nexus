...
LastEditTime: 2024-02-15 10:07:46
...
def get_stock_info(stock:str):
    ...
    stock_info['price'] = round(stock.history(period='1d').Close[0],2) if ('B' not in stock_info['code']) and ('KY' not in stock_info['name']) and (stock_info['stock_class'] != 'ETF') and (stock_info['stock_class'] != "受益證券")else None
    stock_info['prev_day_change'] = round(stock.history(period='2d').diff().Close[1],2) if ('B' not in stock_info['code']) and ('KY' not in stock_info['name']) and (stock_info['stock_class'] != 'ETF') and (stock_info['stock_class'] != "受益證券") else None
